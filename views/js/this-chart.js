...
function add_chip(row, col)
{
    ...
    if(row == FREEROW && col == FREECOL)
    {
        $card.css('background-image', 'url("/assets/piece_free_chip.png")');

    }
    else
    {
        $card.css('background-image', 'url("/assets/piece_chip.png")');
    }
    //$cardtext.hide();
...
function remove_chip(row, col)
{
    ...
    {
        $card.css('background-image', 'url("/assets/piece_free.png")');

    }
    else
    {
        $card.css('background-image', 'url("/assets/piece_bg.png")');
    }
    $cardtext.show();
...
function update_bingo()
{
    ...
    if(bingo)
    {
     $('#yell-link').attr('href', 'https://twitter.com/intent/tweet?hashtags=debatebingo&url='+encodeURI(key_to_link())+"&text="+encodeURIComponent("BINGO!!! Check out my bingo card and make your own"));
        $('#yell-button').attr('src', '/assets/yellbingo_active.png');

    }
    else
    {
        $('#yell-button').attr('src', '/assets/yellbingo_disabled.png');
        $('#yell-link').attr("href", "#");
...
function make_card()
{
    ...
    $('.card').mouseover(function(){
        ...
        {
            $(this).css('background-image', 'url("/assets/piece_free_hover.png")');

        }
        else
        {
            $(this).css('background-image', 'url("/assets/piece_hover.png")');
        }
    ...
    $('.card').mouseout(function(){
        ...
        if(has_chip(splitid[1],splitid[2]))
        {
            if($(this).hasClass('free'))
            {
                $(this).css('background-image', 'url("/assets/piece_free_chip.png")');

            }

            else {
                $(this).css('background-image', 'url("/assets/piece_chip.png")');
            }
            //$('#' + cardid + ' .card-text').hide();
        }
        else if($(this).hasClass('free'))
        {
            $(this).css('background-image', 'url("/assets/piece_free.png")');

        }
        else {
            $(this).css('background-image', 'url("/assets/piece_bg.png")');
        }
    ...
    if(printable)
    {
        $('.free').html('<img src="/assets/print_piece_free.png" />');
    }
    $('.card').click(function(){
        if(printable)
        {
    ...
    $('#yell-button').mouseover(function(){
        ...
        $('#yell-button').attr('src', '/assets/yellbingo_hover.gif');
